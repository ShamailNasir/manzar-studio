/* ============================================================
   LABS HERO - charcoal silk, dark cinema             (engine v7)
   ============================================================
   v6's film-era cues (iridescent rainbow, warm fill, six-channel
   dispersion haze, gate weave, halation glow) added up to a vintage
   cast. v7 returns to the v5 single-pass architecture the page
   looked best in, keeps the one thing v6 got right - normals you
   can actually SEE - and grades the whole frame dark-cinematic:

   - one atlas video (footage + baked normal strip, one clock, so
     the loop can never pop), loop-baked at encode time.
   - normals: the baked strip is near-flat (measured 3.2deg mean
     tilt), so it is amplified in-shader (uNAmp), with a tileable
     silk-weave detail normal (29deg mean tilt) whiteout-blended on
     top [Barre-Brisebois & Hill, "Blending in Detail"], rotated
     30deg off the anisotropic axis and weighted into lit crests.
   - light: ONE cool orbiting key. Wrap diffuse breath, Charlie
     sheen [Estevez & Kulla 2017 - the cloth distribution] kept
     silver, an anisotropic thread gleam, a quiet rim. No warm
     fill, no iridescence - color is not the point; light is.
   - glass: restrained chromatic aberration, three clean taps -
     a small centre floor (uCAc) so the fringe exists mid-frame,
     growing with r^2 to the corners (uCA). Cool-tinted anamorphic
     streaks. Halation faint (film glow reads vintage fast).
   - grade: micro-soften -> clarity (denoise-then-sharpen, the
     colorist order), steel-blue shadows / silver highlights split
     tone, then the ACES filmic curve [Narkowicz fit - the Academy
     tone curve, UE4's default] with an exposure dial and a black
     crush: deep toe, rolled highlights, nothing blows out flat.
   - motion: breathing zoom only. No gate weave - projector wobble
     is a period cue.
...
(function heroSilk () {
  ...
  /* load() resets playbackRate; pin via default AND re-assert. */
  var RATE = 0.85;
  ...
  /* atlas geometry: with FLIP_Y uploads, colour lives at tex v in
     [1/3,1], the normal strip at [0,1/3]. */
  var FS = [
    'precision highp float; varying vec2 vUv;',
    'uniform sampler2D uA, uW;',
    'uniform float uGrainT, uDiff, uSpec, uSpecPow, uAniso, uRim, uGrain, uRM;',
    'uniform float uClar, uSoft, uZoom, uCAc, uCA, uStreak, uHal;',
    'uniform float uNAmp, uDet, uSheen, uSheenR, uExp;',
    ...
    '  st /= uZoom;',
    '  vec2 uv = st + .5;',
    /* chromatic aberration: a small centre floor plus radial growth.
       Three clean taps - a digital-lens fringe, not a rainbow haze. */
    '  vec2 pc = vUv - .5; pc.x *= ca;',
    '  float r2 = dot(pc, pc);',
    '  vec2 cad = normalize(pc + 1e-6) * (uCAc + uCA * r2);',
    /* micro-soften then clarity: denoise-then-sharpen, colorist order */
    '  vec2 sx = uSoft / VRES;',
    '  vec3 base = vid(uv) * .40',
    ...
    '  float l0 = lum(col);',
    /* normals: amplified base + rotated weave detail in the crests */
    ...
    /* one cool key. Wrap diffuse - the breath of the light. */
    '  vec3 L = normalize(uL);',
    '  float d = clamp(dot(N, L) * .5 + .5, 0., 1.);',
    '  d = pow(d, 1.4);',
    '  col *= mix(1. - uDiff * .55, 1. + uDiff * .80, d);',
    /* silver sheen: Charlie lustre + anisotropic thread gleam */
    ...
    /* anamorphic streak: crest light bleeds sideways, kept cool */
    '  float px = 1. / VRES.x;',
    '  float bp = 0.;',
    ...
    /* halation: barely there - film glow reads vintage fast */
    '  vec2 hx = 6.5 / VRES;',
    '  float hl = 0.;',
    ...
    /* the dark-cinema grade: steel shadows / silver highlights,
       then the ACES curve with an exposure dial, then a black crush */
    '  float lm = lum(col);',
  ...
  window.__hero.set = function (o) {
    ...
    return { DIFF: DIFF, SPEC: SPEC, SPECPOW: SPECPOW, ANISO: ANISO, RIM: RIM,
             ...
             TILE: TILE, SHEEN: SHEEN, SHEENR: SHEENR, EXP: EXP, rate: vA.playbackRate };
  };
  ...
  function render (now) {
    ...
    gl.uniform1f(U.uCAc, CAC);
    gl.uniform1f(U.uCA, CA);
    gl.uniform1f(U.uStreak, STREAK);
    gl.uniform1f(U.uHal, HAL);
    gl.uniform1f(U.uNAmp, NAMP);
    gl.uniform1f(U.uDet, DET);
    gl.uniform1f(U.uSheen, SHEEN);
    gl.uniform1f(U.uSheenR, SHEENR);
    gl.uniform1f(U.uExp, EXP);
    gl.uniform2f(U.uRes, cv.width, cv.height);
    gl.uniform2f(U.uTile, TILE * (cv.width / Math.max(cv.height, 1)), TILE);
