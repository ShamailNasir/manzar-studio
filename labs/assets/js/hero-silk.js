...
(function heroSilk () {
  ...
  var FS = [
    ...
    'uniform float uClar, uCurve, uZoom;',
    ...
    /* a slow breath: the frame eases between 1.00 and ~1.045 scale over
       the orbit - motion with no blur and no loss of sharpness */
    '  st /= uZoom;',
    '  vec2 uv = st + .5;',
    '  vec3 col = vid(uv);',
    /* clarity: unsharp micro-contrast. Local contrast in the weave is
       what reads as "high quality" - never softness */
    '  vec2 tx = 1.4 / uVRes;',
    ...
    '  }',
    /* filmic S: toe lifted so the blacks breathe, mids opened,
       highlights rolled - a grade, not a filter */
  ...
  var DIFF = .34, SPEC = .38, SPECPOW = 84., ANISO = .45, RIM = .10,
      LIFT = .55, GRAIN = .034, ORBIT = .10, ELEV = .58,
      CLAR = .55, CURVE = .38;
  ...
  function render (now) {
    ...
    /* the breath: a slow ease in and out of the frame */
    var zoom = 1.0 + .045 * (0.5 + 0.5 * Math.sin(t * .045));
